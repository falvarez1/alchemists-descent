...
//
//   node .claude/skills/deploy-game/deploy.mjs <ref> [--skip-tests] [--keep]
//
...
if (!who.includes(ACCOUNT_ID)) {
  console.error(
    'wrangler is not logged in as an account that owns the project.\n' +
      'Run: node .claude/skills/deploy-game/login.mjs  (browser must be signed in as frankishere@gmail.com)',
  );
  process.exit(1);
}

const sha = out(`git rev-parse --short ${ref}`);
const subject = out(`git log -1 --format=%s ${ref}`);
...
const scratch =
  ...
  join(process.env.LOCALAPPDATA || process.env.TMPDIR || '.', 'Temp', 'alchemists-descent-deploy');
mkdirSync(scratch, { recursive: true });
const wt = join(scratch, `deploy-${sha}`);
if (existsSync(wt)) sh(`git worktree remove --force "${wt}"`);
sh(`git worktree add --detach "${wt}" ${ref}`);

const depsChanged = out(`git diff HEAD ${ref} --stat -- package.json package-lock.json`) !== '';

 ...
 * BOTH mass (kick/blast/inertia response) AND buoyancy: density < 1 floats,
 * > 1 sinks. Material also sets the default colour and the spell-reaction flags.
 */
export interface BodyMaterialDef {
  /** Rapier density (mass = area × density); also density vs water for buoyancy. */
  density: number;
  /** Default packed 0xRRGGBB fill colour. */
  color: number;
  /** Fire ignites and destroys it (P2). */
  flammable: boolean;
  /** Lightning arcs through it (P2). */
  conductive: boolean;
  /** Cell type strewn when it's destroyed, or null. */
  gore: number | null;
}

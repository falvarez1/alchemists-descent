...
const r = await page.evaluate(async () => {
  ...
  const relayDiag = relay ? { rx: relay.x, ry: relay.y, broken0: relay.broken, preFoot, postFoot: footTypes() } : null;
  ...
  p.x = 1540; p.y = 699; p.vx = 0; p.vy = 0; p.fx = 0; p.fy = 0; p.hp = p.maxHp; p.invuln = 0;
  ...
  const trace = [];
  for (let f = 0; f < 260; f++) {
    if (f < 24) { p.x = 1540; p.y = 699; p.vx = 0; p.dead = false; p.hp = p.maxHp; } // stay on the plate until it triggers
    window.__game.tick();
...
check('the victim is shoved toward / into the acid vat (x≥1545) or killed', r.maxX > 1545 || r.dead || r.hpEnd < r.hp0, JSON.stringify({ relayFired: r.relayFired, igniteAt: r.igniteAt, detonatedAt: r.detonatedAt, maxX: r.maxX, hpEnd: r.hpEnd, dead: r.dead }));

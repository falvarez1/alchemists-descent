...
function withCleanSingletons(run: () => void): void {
  ...
  const pacing = { ...PROGRESSION_PACING };
  const gen = { ...GEN_TUNE };
  ...
  try {
    ...
    Object.assign(PROGRESSION_PACING, pacing);
...
describe('tuning persistence', () => {
  it('captures only changed keys (sparse diff vs shipped defaults)', () => {
    withCleanSingletons(() => {
      ...
      PROGRESSION_PACING.enemyStart = PROGRESSION_PACING_DEFAULTS.enemyStart - 0.1;
      ...
      expect(snap.pacing).toEqual({ enemyStart: PROGRESSION_PACING.enemyStart });
  ...
  it('round-trips through storage and ignores other materials/spells', () => {
    withLocalStorage(() => {
      withCleanSingletons(() => {
        ...
        PROGRESSION_PACING.playerStart = 0.61;
        ...
        PROGRESSION_PACING.playerStart = PROGRESSION_PACING_DEFAULTS.playerStart;
        ...
        expect(PROGRESSION_PACING.playerStart).toBe(0.61);
